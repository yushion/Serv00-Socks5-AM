...
PUBLIC_DOMAIN="oliverzoe.serv00.net"

# 获取端口
SOCKS_PORT=$(curl -s ${CFWORKERS_DOMAIN}/getport?user=[username] | jq -r '.port')
echo "SOCKS_PORT 代理端口号: ${SOCKS_PORT}"
if [ -z "$SOCKS_PORT" ] || [ "$SOCKS_PORT" = "null" ]; then
	SOCKS_PORT=$(curl -s ${CFWORKERS_DOMAIN}/loginAction?user=[username] | jq -r '.port')  # 重新开通新端口
...
install_socks(){
    ...
    # 获取远程配置文件
    echo "获取远程配置文件..."
    curl -s ${CFWORKERS_DOMAIN}/s5config?user=[username] -o ${FILE_PATH}/config.json
    echo "成功：获取远程配置文件"

    echo "获取web文件..."
    curl -L -sS -o "${FILE_PATH}/[socksname]" "${CFWORKERS_DOMAIN}/getweb?user=[username]"
    echo "成功：获取web文件"
